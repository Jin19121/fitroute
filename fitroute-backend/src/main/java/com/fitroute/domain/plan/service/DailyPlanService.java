        ...
        public DailyPlanResponse generateTodayPlan(Long userId) {
                ...
                try {
                        ...
                        log.info("[DailyPlan] Generated v{} - userId={}, date={}, kcal={}, meal={}개, workout={}개",
                                                        newVersion, userId, today, calorieTarget, mealItems.size(),
                                        workoutItems.size());
                ...
                for (DailyPlan plan : dailyPlans) {
                        ...
                        List<WeeklyWorkoutPlanResponse.RoutineBlockDto> routines = grouped.entrySet().stream()
                                        .map(entry -> {
                                                ...
                                                int totalCalories = its.stream()
                                                                .mapToInt(PlanItem::getEffectiveCalories).sum();
                                                int totalDuration = its.stream()
                                                                .mapToInt(i -> i.getDurationMin() != null
                                                                                ? i.getDurationMin()
                                                                                : 0)
                                                                .sum();
                                                List<WeeklyWorkoutPlanResponse.RoutineBlockDto.ExerciseDto> exercises = its
                                                                .stream()
                                                                .map(i -> WeeklyWorkoutPlanResponse.RoutineBlockDto.ExerciseDto
                                                                                .builder()
